...
import { Currency, CurrencyAmount, Percent, TradeType, validateAndParseAddress, WETH9 } from 'config/sdk-core';
import { Trade as V2Trade } from 'config/v2-sdk';
import {
  encodeRouteToPath,
  FeeOptions,
  MethodParameters,
...
import { IROUTER_02 } from 'config/abis';
...
export abstract class SwapRouter {
  public static INTERFACE: Interface = IROUTER_02;
  ...
  private static encodeV2Swap(
    ...
    const amountIn: string = toHex(trade.maximumAmountIn(options.slippageTolerance as any).quotient);
    const amountOut: string = toHex(trade.minimumAmountOut(options.slippageTolerance as any).quotient);
  ...
  public static swapAndAddCallParameters(
    ...
    const { chainId } = sampleTrade.route;
