...
import { AVERAGE_L1_BLOCK_TIME } from 'config/constants';
...
export default function usePermit2Allowance(amount?: CurrencyAmount<Token>, spender?: string): Allowance {
  ...
  const { tokenAllowance, isSyncing: isApprovalSyncing } = useTokenAllowance(token, address, PERMIT2_ADDRESS);
  ...
  const { permitAllowance, expiration: permitExpiration, nonce } = usePermitAllowance(token, address, spender);
  ...
  const isPermitted = useMemo(() => {
    ...
    return (
      (permitAllowance.greaterThan(amount as any) || permitAllowance.equalTo(amount as any)) && permitExpiration >= now
    );
