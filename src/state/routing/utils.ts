import { MixedRouteSDK, Protocol, Trade } from 'config/router-sdk';
import { Currency, CurrencyAmount, Token, TradeType } from 'config/sdk-core';
import { Pair, Route as V2Route } from 'config/v2-sdk';
import { FeeAmount, Pool, Route as V3Route } from 'config/v3-sdk';
...
export function computeRoutes(
  ...
  const parsedCurrencyIn = currencyIn.isNative ? Native.onChain(currencyIn.chainId) : parsedTokenIn;
  const parsedCurrencyOut = currencyOut.isNative ? Native.onChain(currencyOut.chainId) : parsedTokenOut;
...
const parsePair = ({ reserve0, reserve1 }: V2PoolInRoute): Pair =>
  new Pair(
    CurrencyAmount.fromRawAmount(parseToken(reserve0.token), reserve0.quotient) as any, // Interface JSBI
    CurrencyAmount.fromRawAmount(parseToken(reserve1.token), reserve1.quotient) as any, // Interface JSBI
  );
...
export function getRouteProtocol(route: (V3PoolInRoute | V2PoolInRoute)[]): Protocol {
  if (route.every((pool) => pool.type === 'v2-pool')) return Protocol.V2;
  if (route.every((pool) => pool.type === 'v3-pool')) return Protocol.V3;
  return Protocol.MIXED;
}

export function getSwapProtocol(trade: Trade<Currency, Currency, TradeType>): Protocol {
  if (trade?.routes.every((pool) => pool.protocol === Protocol.V2)) return Protocol.V2;
  if (trade?.routes.every((pool) => pool.protocol === Protocol.V3)) return Protocol.V3;
  return Protocol.MIXED;
}
